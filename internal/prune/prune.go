...
func Run(ctx context.Context, p Pruner, retentionDays int, now time.Time, log *slog.Logger) error {
	...
	cutoff := partitionAlignedCutoff(now, retentionDays)
...
func partitionAlignedCutoff(now time.Time, retentionDays int) time.Time {
	return startOfUTCDay(now).AddDate(0, 0, -retentionDays)
}
